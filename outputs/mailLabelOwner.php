...
//                            0          1            2         3
$mailLabelFieldsString = "OwnerName,MAILADDLN1,MAILADDLN2,CITYSTZIP";
...
while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_NUMERIC) ) {
    ...
    if(strlen(trim($row[2])) < 2){
		fwrite($ob_file,str_replace(',', '-', $row[0]).",".$row[1].",".$row[3]);   
		fwrite($ob_file,"\r\n");
	}
	else{
		fwrite($ob_file,str_replace(',', '-', $row[0]).",".$row[1].",".$row[2].",".$row[3]);   
		fwrite($ob_file,"\r\n");
	}
}
sqlsrv_close( $conn);
