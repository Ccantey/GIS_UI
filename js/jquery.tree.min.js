...
(function ($, undefined) {
    $.widget("daredevel.tree", {
         ...
         * @param li node
         */
        _indeterminateAncestors: function (li) {
            li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:not(:checked)').prop('indeterminate', true).change();
        },
        ...
        _determinekAncestors: function (li) {
            li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:checked').prop('checked', false).change();
        },

        /**
         * Undetermine ancestors of passed node
         *
         * Don't use uncheck() method because we won't trigger onUncheck events
         *
         * @private
         * @param li node
         */
        _unIndeterminateAncestors: function (li) {
		    //if no siblings, remove indeterminate
			//console.log(li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:checked')); //returns siblings checked after parent unchecked
            // console.log(li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:not(checked)'));
			if (li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:checked').length>0){
			    // console.log('Indeterminate');
			    li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:checked').prop('indeterminate', true).change();
			  }
			  // else if(li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:checked').length==0 ){
			    // console.log('else if: Blank');
			    // li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first').prop('indeterminate', false).change();
			  // }else if(li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:not(:checked)').length>0 ){
			    // console.log('else if: not:checked');
			    // li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first:not(:checked)').prop('indeterminate', true).change();
			  // }
			  else{
			    console.log('ELSE: ')
            ...
            //li.parentsUntil('daredevel-tree').filter('li').find('input:checkbox:first').prop('indeterminate', false).change();
			//else (something else is checked, leave indterminate
        },
        ...
        check: function (li) {
            ...
            if (this.options.onCheck.ancestors == 'check') {
                this._indeterminateAncestors(li); //MYCHANGE
        ...
        uncheck: function (li) {
            ...
            if (this.options.onUncheck.descendants == 'check') {
                ...
                this._uncheckDescendants(li);
				this._unIndeterminateAncestors(li);
				this._determinekAncestors(li);
				//alert('4');
            }
            ...
            if (this.options.onUncheck.ancestors == 'check') {
                ...
                this._uncheckAncestors(li);
				//alert('6');
            }else if (this.options.onUncheck.ancestors != 'uncheck' && this.options.onUncheck.ancestors != 'check'){
			  alert('hj');
			}
        ...
        _markAsCollapsed: function (li) {
            ...
            anchor.removeClass('glyphicon glyphicon-collapse-up');

            if (this.options.collapseUiIcon.length > 0) {
                anchor.addClass('glyphicon glyphicon-expand')
            }
        ...
        _markAsExpanded: function (li) {
            ...
            anchor.removeClass('glyphicon glyphicon-expand');

            if (this.options.expandUiIcon.length > 0) {
                anchor.addClass('glyphicon glyphicon-collapse-up')
            }
